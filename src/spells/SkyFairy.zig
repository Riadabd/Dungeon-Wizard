...
pub const title = "Sky Fairy";

pub const enum_name = "sky_fairy";
...
pub const proto = Spell.makeProto(
    ...
    .{
        ...
        .rarity = .exceptional,
        .color = draw.Coloru.rgb(161, 228, 255).toColorf(),
        .targeting_data = .{
            .kind = .pos,
...
pub fn cast(self: *const Spell, caster: *Thing, room: *Room, params: Params) Error!void {
    ...
    const sky_fairy = self.kind.sky_fairy;
    _ = sky_fairy;
    const target_pos = params.pos;
    const spawner = Thing.SpawnerController.prototypeSummon(.@"fairy-blue");
...
pub fn getTooltip(self: *const Spell, tt: *Spell.Tooltip) Error!void {
    ...
    tt.desc = try Spell.Tooltip.Desc.fromSlice(
        try std.fmt.bufPrint(&tt.desc.buffer, fmt, .{
            ...
            Thing.CreatureKind.@"fairy-blue",
            Spell.Keyword.mislay,
        }),
    );
    tt.pushInfo(.{ .creature = .@"fairy-blue" });
