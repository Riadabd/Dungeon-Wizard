...
pub const title = "Slime Fairy";

pub const enum_name = "slime_fairy";
...
pub const proto = Spell.makeProto(
    ...
    .{
        ...
        .rarity = .exceptional,
        .color = draw.Coloru.rgb(63, 226, 42).toColorf(),
        .targeting_data = .{
            .kind = .pos,
...
pub fn cast(self: *const Spell, caster: *Thing, room: *Room, params: Params) Error!void {
    ...
    const slime_fairy = self.kind.slime_fairy;
    _ = slime_fairy;
    const target_pos = params.pos;
    const spawner = Thing.SpawnerController.prototypeSummon(.@"fairy-green");
...
pub fn getTooltip(self: *const Spell, tt: *Spell.Tooltip) Error!void {
    ...
    tt.desc = try Spell.Tooltip.Desc.fromSlice(
        try std.fmt.bufPrint(&tt.desc.buffer, fmt, .{
            ...
            Thing.CreatureKind.@"fairy-green",
            Spell.Keyword.mislay,
        }),
    );
    tt.pushInfo(.{ .creature = .@"fairy-green" });
